...
export default function Sidebar({ menuItem }: Props) {
    ...
    return (
        <aside className={`xl:w-96 fixed z-50 md:sticky h-screen top-0 transition-all duration-200 ease-in-out ${isOpen ? "w-full md:w-64" : "w-0 md:w-20"}`}>
            <div className={`fixed top-0 left-0 w-full h-full bg-slate-950/15 duration-200 ease-in-out ${isOpen ? "block md:hidden opacity-100" : "hidden opacity-0"}`}></div>
                ...
                <div className="w-full h-[90vh] flex flex-col justify-between overflow-hidden">
                    <div className="w-full h-fit py-5 flex flex-col">
                        {menuItem && menuItem.map((item, id) => (
                            <button key={id} className={`w-full flex items-center xl:gap-4 justify-start px-7 py-2.5 hover:bg-slate-200 md:text-lg duration-200 ease-in-out ${isOpen ? "md:gap-4" : "md:gap-12"}`}>
                    ...
                    <div className="w-full h-fit px-2 xl:px-5 py-5 duration-200 ease-in-out">
                        <div className="w-full h-fit xl:border border-slate-800 rounded-lg duration-200 ease-in-out hover:bg-slate-100">
                            <div className={`w-fit py-3 xl:px-4 flex items-center xl:gap-2 duration-200 ease-in-out ${isOpen ? "gap-2 px-4" : "gap-12 px-1.5"}`}>
                                <div className="w-12 h-12 overflow-hidden flex items-center justify-center rounded-full bg-slate-200">
                                    <UserRound />
                                </div>
                                <div className="flex-1">
                                    <h4 className="text-lg font-semibold leading-5">Unknown user</h4>
